...
export default function Air() {
  ...
  async function checkAirQuality() {
    ...
    let url=`https://api.breezometer.com/air-quality/v2/current-conditions?lat=${position.coords.latitude}&lon=${position.coords.longitude}&key=${process.env.TOKEN2}&features=breezometer_aqi,local_aqi,health_recommendations,sources_and_effects,pollutants_concentrations,pollutants_aqi_information&&metadata=true`;
    try {
      const data = await (
        await fetch(
          url,
          ...
          <div style={{ display: checked ? "block" : "none" }}>
                          ...
                          <div className="flex rounded-lg h-full bg-gray-100 p-8 flex-col">
                            ...
                            <div className="flex-grow">
                              <p className="leading-relaxed text-base">
                                <b>Contentration : </b>
                                {
                                  airData?.data?.pollutants[a].concentration
                                    .value
                                }{" "}
                                {
                                  airData?.data?.pollutants[a].concentration
            ...
            <div className="flex flex-wrap sm:-m-4 -mx-4 -mb-10 -mt-4 md:space-y-0 space-y-6">
              ...
              {airData?.data?.health_recommendations
                ? Object.keys(airData?.data?.health_recommendations).map(
                    (a) => (
